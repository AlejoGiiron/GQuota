-- ============================================================
--  017 — crear_prestamo y crear_prestamo_cuotas guardan el codeudor
--  Ambas RPC reciben y persisten los tres campos del codeudor (opcionales:
--  si vienen null, se guardan null). El resto de su lógica NO cambia.
--
--  Se DROPea la firma anterior antes de recrear: agregar parámetros NO es
--  un "replace" (crea una sobrecarga nueva), y dos firmas distintas con los
--  mismos nombres de argumento dejarían a PostgREST sin poder elegir. Los
--  nuevos parámetros van al final con DEFAULT null para no romper llamadas
--  que no manden codeudor.
-- ============================================================

-- ── crear_prestamo (producto "abierto") ──
drop function if exists public.crear_prestamo(uuid, numeric, numeric, text, date);
...
drop function if exists public.crear_prestamo_cuotas(uuid, numeric, numeric, text, int, date);
...
begin
  ...
  -- meses por cuota: 4 semanas = 1 mes, quincena = ½ mes (igual que el motor).
  ...
  for i in 1..p_n_cuotas loop
    ...
    insert into public.cuotas (
      user_id, prestamo_id, numero, fecha_vence, capital, interes, estado
    ) values (
