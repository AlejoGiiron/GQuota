-- ============================================================
--  012 — RPC crear_prestamo_cuotas
--  Crea de forma atómica un préstamo tipo 'cuotas' + su movimiento de
--  desembolso + TODAS las filas del cronograma (tabla cuotas) con sus
--  fechas de vencimiento. SECURITY INVOKER (la RLS aplica; user_id = auth.uid()).
--
--  REPARTO capital/interés IDÉNTICO a generarCronograma() en
--  src/lib/motor-prestamos.ts (esa es la referencia). Si cambia allá,
--  cambiar aquí. Fechas: desde el desembolso, +7 días (semanal),
--  +15 (quincenal) o +1 mes (mensual) por cuota.
-- ============================================================
...
grant execute on function public.crear_prestamo_cuotas(uuid, numeric, numeric, text, int, date) to authenticated;
