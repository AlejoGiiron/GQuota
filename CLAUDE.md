...
- **Lectura (método A) en el navegador:** `src/lib/lector-cedula.ts` carga zxing con import dinámico; el `.wasm` sale del build (nuestro dominio, sin CDN). Lee a resolución completa ANTES de comprimir, con giros de ±4/8/12°. `src/lib/cedula.ts` devuelve solo número, apellidos, nombres, sexo, fecha y RH y BORRA la huella del buffer (bytes 169+). El contenido crudo nunca sale del navegador. Leído = "Leído de la cédula"; si se edita, pasa a manual; sin respaldo o sin lectura, todo es "Escrito a mano · sin verificar". Sexo y RH solo si se leen. Ningún error de lectura bloquea.
...
- [ ] Ficha por enlace, 1C: revisar la solicitud (el dueño ve las fotos con URL firmada emitida por Edge Function; hoy nadie puede leer el bucket), relectura del respaldo, aprobar como cliente o rechazar. La lista ya muestra "Revisar", "Ver cliente" y "Ver" deshabilitados.
- [ ] (1C, limpieza) Fotos de solicitudes que vencen o se anulan sin enviar quedan en el bucket: borrarlas (job o acción del dueño). Al anular/vencer no hay nada que las borre hoy.
