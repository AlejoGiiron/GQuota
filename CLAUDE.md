...
- "Confirm email" en Supabase Auth está ACTIVO (ON). (Se creía desactivado "para pruebas"; se confirmó el 2026-06-23 que está ON: un signup devuelve `confirmation_sent_at` y NO entrega sesión. Implicación práctica: no se puede mintear un JWT de usuario sin acceso al buzón, así que las Edge Functions con guard de rol no se pueden probar E2E sin cuentas reales. OJO al crear cobradores: `crear-cobrador` pone `email_confirm: true`, así que el cobrador entra sin confirmar correo aunque esto esté ON.)
- [ ] Verificación E2E de la Fase 4 (equipo) en UI con cuentas LAB (dueño + cobrador): crear cobrador → entra con su clave; reset de clave; quitar → no entra y no ve nada → reactivar → entra con la clave anterior; cambiar rol; y los guards (cobrador → 403, frontera entre negocios, no quitar/degradar al último dueño activo). El backend está desplegado; falta probarlo con sesiones reales (bloqueado por Confirm email ON / no hay claves de prueba en esta sesión).
