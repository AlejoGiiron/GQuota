...
- [ ] (prioridad ALTA) "Olvidé contraseña": hoy muestra "disponible pronto" (src/pages/LoginPage.tsx). Con el registro público abierto, un usuario que olvida la clave queda fuera sin salida. Depende de configurar Resend (o SMTP) en Supabase Auth para enviar el correo de recuperación.
...
- Estado del release (2026-06-23, ACTUAL): `main` en `c10e735` (merge `--no-ff` de `develop`, que está en `87bec2e`; mismo árbol, verificado el 2026-09-23 con `git diff origin/develop origin/main` vacío), desplegado a producción. **Las migraciones aplicadas en producción llegan hasta la 034** (según el cierre de la Fase 5; falta reconfirmarlo con `supabase migration list` desde el equipo nuevo). Agrega la **Fase 5 completa (registro self-service)**: migración 034, RPC `crear_mi_negocio(nombre)` (SECURITY DEFINER, atómica: negocio + membresía de dueño; guard "un usuario, un negocio" reforzado por el UNIQUE de `miembros.user_id`; EXECUTE revocado a anon); pantallas `RegistroPage`, `SinNegocioPage` (recuperación si el signUp quedó OK pero la RPC falló) y onboarding en `InicioPage` (estado vacío que invita a crear el primer cliente/préstamo). El registro público queda ABIERTO (sin tope ni cobro todavía). Con esto las Fases 1–5 del SaaS están en producción. Próximo release: avanzar `main` con lo que entre a `develop`.
...
- "Confirm email" en Supabase Auth está ACTIVO (ON) (visto en el panel por el dueño el 2026-09-23). Un signup NO entrega sesión hasta confirmar el correo: `RegistroPage` muestra "revisa tu correo" y, al entrar tras confirmar, `SinNegocioPage` termina de crear el negocio. Implicación: el registro público DEPENDE de que el correo de confirmación llegue; mientras no haya Resend/SMTP propio, verificar que el correo por defecto de Supabase entregue a direcciones externas (tiene límites de envío bajos). OJO al crear cobradores: `crear-cobrador` pone `email_confirm: true`, así que el cobrador entra sin confirmar correo sin importar este ajuste.
