...
- [ ] (v2, deuda de modelo) Detalle pactado por cuota: al pagar una cuota la RPC vacía su capital/interés a 0 y `movimientos` no referencia la cuota (no hay cuota_id/numero, y un pago puede tocar varias cuotas), así que el cronograma no puede mostrar el monto original por cuota pagada. Hoy se muestra "Pagada/—" + la sección "Pagos realizados" desde movimientos (honesto pero sin desglose por cuota). Arreglo de fondo: agregar numero/cuota_id a `movimientos`, o no vaciar la cuota al pagar.