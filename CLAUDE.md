...
- [ ] (seguridad, baja) `npx supabase db advisors --linked --type security` (2026-09-24) marca 7 funciones SECURITY DEFINER ejecutables por `anon`: los helpers de RLS (`mi_negocio`, `mi_rol`, `mi_miembro_id`, `puedo_ver_prestamo`, `cliente_visible_para_cobrador`, `es_miembro_activo_del_negocio`) y `rls_auto_enable`, que NO sale de ninguna migración del repo (averiguar de dónde viene). Sin sesión devuelven null/false, pero `es_miembro_activo_del_negocio` responde a quien tenga los UUID. Revocar EXECUTE a `anon` (las políticas corren como `authenticated`) tras probarlo en seco.
