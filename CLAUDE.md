...
- **Edge Function `ficha-publica`** (pública): calcula el hash y busca con service_role; responde `activa` (nombre del negocio, ficha_config, nombre de referencia, vencimiento) · `vencida` · `usada` · `no_disponible` (inexistente, mal formado o anulado: el mismo mensaje). No registra tokens. **Pública, sin verificación de JWT:** lo fija `supabase/config.toml` (`[functions.ficha-publica] verify_jwt = false`), así que se despliega con `npx supabase functions deploy ficha-publica --use-api`, sin bandera. Verificado: responde 200 sin cabecera de autenticación, mientras `gestionar-equipo` sigue en 401.
...
- [ ] Ficha por enlace: 1B (formulario del prospecto, fotos, lectura de la cédula, autorización) y 1C (revisar, aprobar como cliente o rechazar). En la 1A la lista ya muestra "Revisar", "Ver cliente" y "Ver" deshabilitados, y "Empezar" en la página pública dice "Disponible pronto". El diseño pide un botón "Escribir al WhatsApp del negocio" en las pantallas de enlace vencido/usado, pero el negocio aún no guarda su WhatsApp (llega con "Mi marca").
